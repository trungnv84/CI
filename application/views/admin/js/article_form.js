...
$(document).ready(function() {
    ...
    $("#start_date, #end_date, #show_date").datepicker({
    ...
    admin.validate = function() {
        ...
        var str = $.trim(el.val());
        if(str=="") {
            admin.invalid(el, "Bạn phải nhập tiêu đề.");
            result = false;
        }
        var el = $("#cat_ids");
        var str = el.find("option:selected");
        if(str.length==0) {
            admin.invalid(el, "Bạn phải chọn ít nhất một loại tin.");
            result = false;
        }
        return result;
    };
